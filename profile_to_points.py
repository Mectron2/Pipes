...
def detect_grid(crop_gray: np.ndarray) -> tuple[np.ndarray, list[int], list[int]]:
    mask = dark_mask(crop_gray)
    h, w = mask.shape

    horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (max(40, w // 28), 1))
    vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, max(40, h // 18)))
    ...
    row_hits = np.count_nonzero(horizontal, axis=1)
    col_hits = np.count_nonzero(vertical, axis=0)
    rows = grouped_positions(row_hits, minimum=w * 0.25)
    cols = grouped_positions(col_hits, minimum=h * 0.25)
...
def parse_profile_image(image_path: Path, debug_dir: Path | None, epsilon: float) -> dict:
    ...
    write_debug(debug_dir, "06_overlay.png", draw_overlay(image, bounds, centerline, simplified))
